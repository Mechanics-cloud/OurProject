...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3530
        NAMESPACE = "car-robot-ru"
        REGISTRY_HOSTNAME = "alexanderekb"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "robots-progect"
        DEPLOYMENT_NAME = "robots-progect-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'car-robot-ru') {
